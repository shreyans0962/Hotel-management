		 ...
		 PreparedStatement ps = null;
		    ResultSet result = null;
				try {
			    		...
			    		GetConnection connect=new GetConnection();
			    		Connection conn=connect.getConnection();
			        	 ps=conn.prepareStatement("INSERT INTO roomcustomer(name,address,phone,roomtype,bedtype,price,status) VALUES (?,?,?,?,?,?,?)");
	...
	public void checkOut()
	{
	    ...
	    ResultSet result = null;
			try {
		    		...
		    		GetConnection connect=new GetConnection();
		    		Connection conn=connect.getConnection();
		        	 ps=conn.prepareStatement("UPDATE roomcustomer SET status = 1 where name=?");
		        	 ps.setString(1,name.getText());
